...
import { addFavMovie } from "../context/MovieSlice";
...
const Details = () => {
  ...
  const { data: acters } = useQuery({
    queryKey: ["Acting", id],
    queryFn: () => GetActing(id),
  });
  const { data: similair } = useQuery({
    queryKey: ["similair", id],
    queryFn: () => GetSimilar(id),
  });
  const { data: Reviews } = useQuery({
    queryKey: ["reviews", id],
    queryFn: () => GetVideoReviews(id),
  });
  if (isLoading)
    return (
      <div>
        ...
        <div className="px-8 py-28 max-w-6xl relative z-40 mx-auto flex gap-6">
          ...
          <div className="flex-[2] flex flex-col gap-6">
            ...
            <div className=" flex items-center justify-center space-x-10">
              ...
              {/* <BookmarkCheck className="cursor-pointer hover:text-red-500" /> */}
              <Heart
                onClick={() => dispatch(addFavMovie(details))}
