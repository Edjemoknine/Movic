...
import { addFavTv, removeFavTv } from "../context/MovieSlice";
const TvCard = ({ card }) => {
  const dispatch = useDispatch();
  const { favoriteTv } = useSelector((store) => store.movie);
  const saved = favoriteTv.find((fav) => fav.id === card.id);
  ...
  return (
    <div className="hover:z-10 h-64 relative group overflow-hidden hover:scale-110 duration-300 cursor-pointer mx-1 bg-base-100 shadow-xl">
      ...
      <div className="absolute top-6 group-hover:right-7 -right-6 flex gap-3 flex-col duration-300">
        <Heart
          onClick={() => dispatch(addFavTv(card))}
          className={`${
            saved && "text-red-500"
          } z-20 relative cursor-pointer hover:text-red-500`}
        />
        <Trash2
          onClick={() => dispatch(removeFavTv(card.id))}
          className={`${
            saved ? "flex" : "hidden"
          } z-20 relative cursor-pointer hover:text-red-500`}
        />
      ...
      <div className=" z-20 p-2 absolute -bottom-40 group-hover:bottom-10 duration-300 flex-col flex gap-3 text-white left-3  text-centr">
